...
| 이름 | 회고 |
|------|------|
| 김종민 | 스포티파이 데이터로 콘텐츠 기반 음악 추천 시스템을 구축하면서, 데이터 전처리의 중요성과 사용자 관점에서의 기능 설계가 얼마나 중요한지 깊이 체감했습니다.
특히 KNN과 코사인 유사도를 활용한 추천 알고리즘 구현 과정에서 이론과 실제 구현 사이의 간극을 메우는 경험을 할 수 있었고, 향후 협업 필터링을 추가한 하이브리드 모델로 발전시켜보고 싶습니다. |
